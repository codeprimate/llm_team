      ...
      module ResearchAgent
        ...
        class SymbolicMathAgent < LlmTeam::Core::Agent
          ...
          def normalize_expression(expr)
            ...
            normalized.gsub(/\s+/, " ").strip  # normalize whitespace
          end
          ...
          def simplify_with_symbo(expression)
            ...
            apply_basic_simplifications(expression)
          rescue => e
          ...
          def apply_basic_simplifications(expression)
            ...
            # (ax + b)/c → ax/c + b/c
            if simplified =~ /\((\d+)x\+(\d+)\)\/(\d+)/
              coeff = $1.to_i
              const = $2.to_i
              divisor = $3.to_i
              simplified = if coeff % divisor == 0 && const % divisor == 0
                "#{coeff / divisor}x + #{const / divisor}"
              else
                "#{coeff}/#{divisor}*x + #{const}/#{divisor}"
              end
            end
