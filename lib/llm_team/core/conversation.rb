  ...
  module Core
    ...
    class Conversation
      ...
      def build_conversation_for_llm(system_prompt, new_user_message, history_behavior: nil)
        ...
        messages << {role: LlmTeam::ROLE_ASSISTANT, content: "--- The ACTUALcurrent date and time is #{Time.now.strftime("%Y-%m-%d %H:%M:%S")}. Base your responses on this actual time and date. ---\n\n"}
